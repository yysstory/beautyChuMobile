...
function loadStyleList(pageNo) {
	...
	$.getJSON('../json/style/list.do?pageNo=' + pageNo, function(data) {
		...
		$('#styletemplate').load('../templates/styletemplate.html',
				function() {
					if (data.status === 'success') {
						for (var i = 0; i < styleList.length; i++) {
							...
							if ($('#content-section-1').outerHeight() <= $('#content-section-2').outerHeight()) {
...
$(document).on("pageinit", "#styledetail", function() {
	...
	$(function() {
		...
		$.getJSON('../json/style/styledetail.do?styleNo=' + styleNo[styleNo.length-1], function(data) {
			$('#styletemplate').on('load','../templates/styledetailtemplate.html',
					function() {
								var source = $('#styletemplate').html();
								var template = Handlebars.compile(source);
								var html = template(data);
									$('#styledetail').append(html);
			});
		})
	})
})
